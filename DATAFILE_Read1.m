function [D,R] = DATAFILE_Read1(FileName,Trial0,Trial1,FrameDataFlag)
% Reads experimental data for trials from a file (Version 1).
%   [D,R] = DATAFILE_Read1(FileName,Trial0,Trial1,FrameDataFlag)
%   D - Data
%   R - Return code (Boolean success).

%FileName = 'nc-05sep2007-0916.dat';
%Trial0 = 100;
%Trial1 = 200;
%nargin = 3;
%nargout = 1;
...
for v=1:TrialVariables
    ...
    s = fread(FID,DATAFILE_STRLEN,'uint8=>char')';
    TrialVariableName{v} = sprintf('%s',s);
end
...
for f=1:FrameBlocks
    ...
    for v=1:FrameVariables(f)
        ...
        s = fread(FID,DATAFILE_STRLEN,'uint8=>char')';
        FrameVariableName{f,v} = sprintf('%s',s);
    end
end
...
while( EOF == 0 )
    ...
    if( DataType ~= DATAFILE_TRIAL )
        fclose(FID);
        disp(sprintf('File parsing error (Expected DataType=TRIAL): %s',FileName));
        D = [ ]; R = 0;
        return;
    end
